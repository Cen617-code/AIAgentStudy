...
researcher_agent_1 = LlmAgent(
    ...
    model=GEMINI_MODEL,
    instruction="""You are an AI Research Assistant specializing in energy. Research the latest advancements in 'renewable energy sources.' Use the Google Search provided. Summarize your key findings concisely (1-2 sentences). Output *only* the summary.""",
    description="Researchers renewalbe energy sources.",
    tools=[google_search],
...
merger_agent = LlmAgent(
    ...
    description="Combines research findings from paraller agents into a structured, cited report, strictly grounded on provied inputs",
)   

sequential_pipe_agent = SequentialAgent(
    name="ResearchAndSynthesisPipeline",
    sub_agents=[parallel_research_agent, merger_agent],
    description="Coordinated parallel research and synthesizes the results."
)

root_agent = sequential_pipe_agent